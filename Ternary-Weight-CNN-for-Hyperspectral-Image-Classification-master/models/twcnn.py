...
from models.modules import DenseClassifier, TernaryWeightLayer


class FeatureExtractionCNN(nn.Module):
    def __init__(self, in_channels: int) -> None:
        super().__init__()
        self.conv1_spectral = nn.Conv2d(in_channels, 32, kernel_size=1)
        self.bn1_spectral = nn.BatchNorm2d(32)
        
        self.conv1_spatial = nn.Conv2d(in_channels, 32, kernel_size=3, padding='same')
        self.bn1_spatial = nn.BatchNorm2d(32)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        x1 = F.relu(self.bn1_spectral(self.conv1_spectral(x)))
        x2 = F.relu(self.bn1_spatial(self.conv1_spatial(x)))
